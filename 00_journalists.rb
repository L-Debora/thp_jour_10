...
puts "combien y-a t'il de journaliste dans cet array ?"
...
puts "il y a #{journalist_array.length} dans mon tableau"

# il y à 379 journalistes dans le tableau
...
journalist_array.each do |email|
    if email.match(/[0-9]/) #match tout les chiffres dans le tableau.
        count += 1 #nombre de mails trouvés
    end
end
...
journalist_array.each do |email|
    if email.match(/@[A-Z]/) #il prend en premier l'@ et ensuite il rentre dans les crochets
        count += 1 
    end
end
puts "#{count}"
...
puts "Voici la liste alphabétique :#{journalist_array.sort}" #.sort permet de trier alphabétiquement le tableau

#Question 7 :
puts "Quels sont les 50 handles les plus courts de ce array ?"

sorted = journalist_array.sort_by {|word| word.length}

puts sorted[0..49] # cf slice

#Question 8 :
puts "Quelle est la position dans l'array de la personne @epenser ?"

puts journalist_array.index("@epenser")
